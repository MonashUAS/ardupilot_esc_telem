...
void AP_EFI::send_mavlink_status(mavlink_channel_t chan)
{
    ...
    mavlink_msg_efi_status_send(
        ...
        KELVIN_TO_C(state.cylinder_status[0].exhaust_gas_temperature),      // exhaust gas temperature
        state.throttle_output,                                              // throttle out
        0,                                                                  // pressure/temperature compensation
        ignition_voltage                                                    // ignition voltage (spark supply voltage)
    );
}
