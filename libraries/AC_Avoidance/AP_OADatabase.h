...
public:
    ...
    const DbItem& get_item(uint32_t i) const { return _database.items[i]; }
...
private:
    ...
    void database_item_add(const DbItem &item);
    ...
    uint8_t get_send_to_gcs_flags(const DbItemImportance importance);

    // returns true if database item "index" is close to "item"
    bool is_close_to_item_in_database(const uint16_t index, const DbItem &item) const;
    ...
    struct {
        ObjectBuffer<DbItem> *items;                        // thread safe incoming queue of points from proximity sensor to be put into database
    ...
    struct {
        DbItem       *items;                             // array of objects in the database
