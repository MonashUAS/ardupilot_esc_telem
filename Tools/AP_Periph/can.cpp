...
static void handle_param_getset(CanardInstance* canard_instance, CanardRxTransfer* transfer)
{
    ...
    uint16_t total_size = uavcan_protocol_param_GetSetResponse_encode(
        &pkt,
        buffer
#if CANARD_ENABLE_TAO_OPTION
        , !periph.canfdout()
...
static void can_safety_button_update(void)
{
    ...
    uint16_t total_size = ardupilot_indication_Button_encode(
        &pkt,
        buffer
#if CANARD_ENABLE_TAO_OPTION
        , !periph.canfdout()
    ...
    if (periph.debug_option_is_set(AP_Periph_FW::DebugOptions::ENABLE_STATS)) {
        {
            ...
            uint32_t len = dronecan_protocol_Stats_encode(
                &protocol_stats,
                buffer
#if CANARD_ENABLE_TAO_OPTION
                , !periph.canfdout()
        ...
        for (auto &ins : instances) {
            ...
            uint32_t len = dronecan_protocol_CanStats_encode(
                &can_stats,
                buffer
#if CANARD_ENABLE_TAO_OPTION
                , !periph.canfdout()
...
void AP_Periph_FW::pwm_hardpoint_update()
{
    ...
    if (value > 0 && now - pwm_hardpoint.last_send_ms >= 1000U/rate) {
        ...
        uint16_t total_size = uavcan_equipment_hardpoint_Command_encode(
            &cmd,
            buffer
#if CANARD_ENABLE_TAO_OPTION
            , !periph.canfdout()
...
void AP_Periph_FW::can_mag_update(void)
{
    ...
    uint16_t total_size = uavcan_equipment_ahrs_MagneticFieldStrength_encode(
        &pkt,
        buffer
#if CANARD_ENABLE_TAO_OPTION
        , !periph.canfdout()
...
void AP_Periph_FW::can_battery_update(void)
{
    ...
    for (uint8_t i=0; i<battery_instances; i++) {
        ...
        const uint16_t total_size = uavcan_equipment_power_BatteryInfo_encode(
            &pkt,
            buffer
#if CANARD_ENABLE_TAO_OPTION
            , !periph.canfdout()
...
void AP_Periph_FW::can_gps_update(void)
{
...

    {
        ...
        */
        ...
        uint16_t total_size = uavcan_equipment_gnss_Fix2_encode(
            &pkt,
            buffer
#if CANARD_ENABLE_TAO_OPTION
            , !periph.canfdout()
     ...
     */
    {
        ...
        uint16_t total_size = uavcan_equipment_gnss_Auxiliary_encode(
            &aux,
            buffer
#if CANARD_ENABLE_TAO_OPTION
            , !periph.canfdout()
    ...
    if (gps.have_gps_yaw() && last_relposheading_ms == 0) {
        ...
        if (gps.gps_yaw_deg(yaw_deg, yaw_acc_deg, yaw_time_ms) && yaw_time_ms != last_gps_yaw_ms) {
            ...
            const uint16_t total_size = ardupilot_gnss_Heading_encode(
                &heading,
                buffer
#if CANARD_ENABLE_TAO_OPTION
                , !periph.canfdout()
...
void AP_Periph_FW::send_moving_baseline_msg()
{
    ...
    const uint16_t total_size = ardupilot_gnss_MovingBaselineData_encode(
        &mbldata,
        buffer
#if CANARD_ENABLE_TAO_OPTION
        , !periph.canfdout()
...
void AP_Periph_FW::send_relposheading_msg() {
    ...
    const uint16_t total_size = ardupilot_gnss_RelPosHeading_encode(
        &relpos,
        buffer
#if CANARD_ENABLE_TAO_OPTION
        , !periph.canfdout()
...
void AP_Periph_FW::can_baro_update(void)
{
...

    {
        ...
        uint16_t total_size = uavcan_equipment_air_data_StaticTemperature_encode(
            &pkt,
            buffer
#if CANARD_ENABLE_TAO_OPTION
            , !periph.canfdout()
...
void AP_Periph_FW::can_airspeed_update(void)
{
    ...
    uint16_t total_size = uavcan_equipment_air_data_RawAirData_encode(
        &pkt,
        buffer
#if CANARD_ENABLE_TAO_OPTION
        , !periph.canfdout()
...
void AP_Periph_FW::can_rangefinder_update(void)
{
    ...
    uint16_t total_size = uavcan_equipment_range_sensor_Measurement_encode(
        &pkt,
        buffer
#if CANARD_ENABLE_TAO_OPTION
        , !periph.canfdout()
...
void AP_Periph_FW::can_efi_update(void)
{
...

    {
        ...
        */
        ...
        const uint16_t total_size = uavcan_equipment_ice_reciprocating_Status_encode(
            &pkt,
            buffer
#if CANARD_ENABLE_TAO_OPTION
            , !periph.canfdout()
...
void can_printf(const char *fmt, ...)
{
    ...
    for (uint8_t i=0; i<packet_count_max && char_count > 0; i++) {
        ...
        const uint32_t len = uavcan_protocol_debug_LogMessage_encode(
            &pkt,
            buffer_packet
#if CANARD_ENABLE_TAO_OPTION
            , !periph.canfdout()
    ...
    uint32_t len = uavcan_protocol_debug_LogMessage_encode(
        &pkt,
        buffer
#if CANARD_ENABLE_TAO_OPTION
        , !periph.canfdout()
