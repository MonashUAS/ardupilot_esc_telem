...
bool AP_Arming_Copter::barometer_checks(bool display_failure)
{
    ...
    bool ret = true;
    // check Baro
    if ((checks_to_perform == AP_Arming::Check::ALL) || (checks_to_perform & AP_Arming::Check::BARO)) {
        ...
        if (using_baro_ref) {
            if (fabsf(copter.inertial_nav.get_altitude() - copter.baro_alt) > PREARM_MAX_ALT_DISPARITY_CM) {
                check_failed(AP_Arming::Check::BARO, display_failure, "Altitude disparity");
                ret = false;
            }
        }
    }
...
bool AP_Arming_Copter::compass_checks(bool display_failure)
{
    ...
    if ((checks_to_perform == AP_Arming::Check::ALL) || (checks_to_perform & AP_Arming::Check::COMPASS)) {
        ...
        if (!AP::compass().configured()) {
            check_failed(AP_Arming::Check::COMPASS, display_failure, "Compass not calibrated");
            ret = false;
        }
    }
...
bool AP_Arming_Copter::ins_checks(bool display_failure)
{
    ...
    if ((checks_to_perform == AP_Arming::Check::ALL) || (checks_to_perform & AP_Arming::Check::INS)) {

        // get ekf attitude (if bad, it's usually the gyro biases)
        if (!pre_arm_ekf_attitude_check()) {
            check_failed(AP_Arming::Check::INS, display_failure, "EKF attitude is bad");
            ret = false;
        }
    }
...
bool AP_Arming_Copter::board_voltage_checks(bool display_failure)
{
    ...
    if ((checks_to_perform == AP_Arming::Check::ALL) || (checks_to_perform & AP_Arming::Check::VOLTAGE)) {
        if (copter.battery.has_failsafed()) {
            check_failed(AP_Arming::Check::VOLTAGE, display_failure, "Battery failsafe");
            return false;
        }
...
bool AP_Arming_Copter::parameter_checks(bool display_failure)
{
    ...
    if ((checks_to_perform == AP_Arming::Check::ALL) || (checks_to_perform & AP_Arming::Check::PARAMETERS)) {

        // ensure all rc channels have different functions
        if (rc().duplicate_options_exist()) {
            check_failed(AP_Arming::Check::PARAMETERS, display_failure, "Duplicate Aux Switch Options");
            return false;
        }
        ...
        if (copter.g.failsafe_throttle) {
            ...
            if (copter.channel_throttle->get_radio_min() <= copter.g.failsafe_throttle_value+10 || copter.g.failsafe_throttle_value < 910) {
                check_failed(AP_Arming::Check::PARAMETERS, display_failure, "Check FS_THR_VALUE");
                return false;
            }
        }
        ...
        if (copter.aparm.angle_max < 1000 || copter.aparm.angle_max > 8000) {
            check_failed(AP_Arming::Check::PARAMETERS, display_failure, "Check ANGLE_MAX");
            return false;
        }
        ...
        if ((copter.g.acro_balance_roll > copter.attitude_control->get_angle_roll_p().kP()) || (copter.g.acro_balance_pitch > copter.attitude_control->get_angle_pitch_p().kP())) {
            check_failed(AP_Arming::Check::PARAMETERS, display_failure, "ACRO_BAL_ROLL/PITCH");
            return false;
        }
#endif
        ...
        if (copter.g2.frame_class.get() != AP_Motors::MOTOR_FRAME_HELI_QUAD &&
            ...
            check_failed(AP_Arming::Check::PARAMETERS, display_failure, "Invalid Heli FRAME_CLASS");
            return false;
        }

        // check helicopter parameters
        if (!copter.motors->parameter_check(display_failure)) {
            check_failed(AP_Arming::Check::PARAMETERS, display_failure, "Heli motors checks failed");
            return false;
        }
        ...
        if (copter.g2.frame_class.get() != AP_Motors::MOTOR_FRAME_HELI_QUAD &&
            ...
            check_failed(AP_Arming::Check::PARAMETERS, display_failure, "Inverted flight option not supported");
            return false;
        }
        ...
        if (rc().find_channel_for_option(RC_Channel::aux_func_t::MOTOR_INTERLOCK) == nullptr) {
            check_failed(AP_Arming::Check::PARAMETERS, display_failure, "Motor Interlock not configured");
            return false;
        }
        ...
        if (copter.g2.frame_class.get() == AP_Motors::MOTOR_FRAME_HELI_QUAD ||
            copter.g2.frame_class.get() == AP_Motors::MOTOR_FRAME_HELI_DUAL ||
            copter.g2.frame_class.get() == AP_Motors::MOTOR_FRAME_HELI) {
            check_failed(AP_Arming::Check::PARAMETERS, display_failure, "Invalid MultiCopter FRAME_CLASS");
            return false;
        }
        #endif // HELI_FRAME
        ...
        if (copter.failsafe.adsb) {
            check_failed(AP_Arming::Check::PARAMETERS, display_failure, "ADSB threat detected");
            return false;
        }
#endif
...
bool AP_Arming_Copter::pilot_throttle_checks(bool display_failure)
{
    ...
    if ((checks_to_perform == AP_Arming::Check::ALL) || (checks_to_perform & AP_Arming::Check::RC)) {
        if (copter.g.failsafe_throttle != FS_THR_DISABLED && copter.channel_throttle->get_radio_in() < copter.g.failsafe_throttle_value) {
            ...
            #endif
            check_failed(AP_Arming::Check::RC, display_failure, "%s", failmsg);
            return false;
        }
    }
...
bool AP_Arming_Copter::gps_checks(bool display_failure)
{
    ...
    if (copter.gps.get_hdop() > copter.g.gps_hdop_good) {
        check_failed(AP_Arming::Check::GPS, display_failure, "High GPS HDOP");
        return false;
    }
...
bool AP_Arming_Copter::pre_arm_terrain_check(bool display_failure)
{
    ...
    if (copter.rangefinder_state.enabled && (copter.g.rtl_altitude > copter.rangefinder.max_distance_cm_orient(ROTATION_PITCH_270))) {
        check_failed(AP_Arming::Check::PARAMETERS, display_failure, "RTL_ALT above rangefinder max range");
        return false;
    }
    ...
    if (!have_all_data) {
        check_failed(AP_Arming::Check::PARAMETERS, display_failure, "Waiting for Terrain data");
    }
...
bool AP_Arming_Copter::proximity_checks(bool display_failure) const
{
    ...
    if (copter.avoid.proximity_avoidance_enabled() && copter.g2.proximity.get_closest_object(angle_deg, distance)) {
        ...
        if (distance <= 0.6f) {
            check_failed(AP_Arming::Check::PARAMETERS, display_failure, "Proximity %d deg, %4.2fm", (int)angle_deg, (double)distance);
            return false;
        }
    }
#endif
...
bool AP_Arming_Copter::arm_checks(AP_Arming::Method method)
{
    ...
    if ((checks_to_perform == AP_Arming::Check::ALL) || (checks_to_perform & AP_Arming::Check::INS)) {
        if (degrees(acosf(ahrs.cos_roll()*ahrs.cos_pitch()))*100.0f > copter.aparm.angle_max) {
            check_failed(AP_Arming::Check::INS, true, "Leaning");
            return false;
        }
    }
    ...
    if ((checks_to_perform == AP_Arming::Check::ALL) || (checks_to_perform & AP_Arming::Check::PARAMETERS)) {
        if (copter.failsafe.adsb) {
            check_failed(AP_Arming::Check::PARAMETERS, true, "ADSB threat detected");
            return false;
        }
    }
#endif
    ...
    if ((checks_to_perform == AP_Arming::Check::ALL) || (checks_to_perform & AP_Arming::Check::RC)) {
        ...
        if (copter.g.failsafe_throttle != FS_THR_DISABLED && copter.channel_throttle->get_radio_in() < copter.g.failsafe_throttle_value) {
            check_failed(AP_Arming::Check::RC, true, "%s below failsafe", rc_item);
            return false;
        }
        ...
        if (!(method == AP_Arming::Method::MAVLINK && (control_mode == Mode::Number::GUIDED || control_mode == Mode::Number::GUIDED_NOGPS))) {
            ...
            if (copter.get_pilot_desired_climb_rate(copter.channel_throttle->get_control_in()) > 0.0f) {
                check_failed(AP_Arming::Check::RC, true, "%s too high", rc_item);
                return false;
            }
            // in manual modes throttle must be at zero
            #if FRAME_CONFIG != HELI_FRAME
            if ((copter.flightmode->has_manual_throttle() || Mode::Number::DRIFT) && copter.channel_throttle->get_control_in() > 0) {
                check_failed(AP_Arming::Check::RC, true, "%s too high", rc_item);
                return false;
            }
            #endif
